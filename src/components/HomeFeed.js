...
const allStories = [
  ...
  },
  {
    id: 2,
    user: "@alex",
    text: "wore this to a halloween party as a pirate!",
    image: null,
    itemName: "Red scarf",
    from: "@emily",
    to: "@alex",
    ...
    <div className="home-feed">
      ...
      {stories.map((story) => (
        <div key={story.id} className="feed-post-container">
          ...
          >
            <span style={{ color: "#222" }}>{story.from}</span>{" "}
            <span style={{ fontSize: "1.1em" }}>→</span>{" "}
            <span style={{ color: "#222" }}>{story.to}</span>{" "}
          ...
          <div className="swap-actions">
            <button onClick={() => navigate(`/item/${story.id}`)}>
              {" "}
              <FontAwesomeIcon icon={faEye} /> view item
            </button>
            <button onClick={() => navigate(`/item/${story.id}/history`)}>
              {" "}
